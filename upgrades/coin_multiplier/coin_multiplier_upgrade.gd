...
func _on_cash_changed(_new_total: int, amount_added: int) -> void:
	...
	if amount_added > 0:
		...
		if bonus_amount > 0:
			...
			level_controller.cash_manager.cash_changed.disconnect(_on_cash_changed)
			level_controller.cash_manager.add_cash(bonus_amount)
			level_controller.cash_manager.cash_changed.connect(_on_cash_changed)
...
func _on_game_over() -> void:
	...
	if (
		level_controller.cash_manager
		and level_controller.cash_manager.cash_changed.is_connected(_on_cash_changed)
	):
		level_controller.cash_manager.cash_changed.disconnect(_on_cash_changed)
